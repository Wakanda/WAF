...
Event.create('DataSource');
Event.create('AttributeChange', Event.DataSource);
Event.create('CurrentElementChange', Event.AttributeChange);
Event.create('CollectionChange', Event.CurrentElementChange);

Event.create('ElementSaved', Event.DataSource);
Event.create('BeforeCurrentElementChange', Event.DataSource);

WAF.DataSourceEmSimpleAttribute = Class.create('DataSourceEmSimpleAttribute');
...
WAF.DataSourceEmSimpleAttribute.addListener = function(eventHandler, options, userData){
	...
	userData = userData;
	return this.owner.addListener(Event.AttributeChange, eventHandler, options, userData);
};
...
WAF.DataSourceEmSimpleAttribute.unsubscribe = function(event, target, callback, observer){
	this.owner.unsubscribe(event, target, callback, observer);
};

WAF.DataSourceEmSimpleAttribute._removeSubscribersInList = function(identifier, event, target, callback, observer){
	this.owner._removeSubscribersInList(identifier, event, target, callback, observer);
};

WAF.DataSourceEmSimpleAttribute.unsubscribeEvent = function(event, target){
	this.owner.unsubscribeEvent(event, target);
};

WAF.DataSourceEmSimpleAttribute.unsubscribeCallback = function(callback){
	this.owner.unsubscribeCallback(callback);
};

WAF.DataSourceEmSimpleAttribute.unsubscribeObserver = function(observer){
	this.owner.unsubscribeObserver(observer);
};

WAF.DataSourceEmSimpleAttribute.fire = function(event, target, data){
	this.owner.fire(event, target, data);
};
...
WAF.DataSourceEmSimpleAttribute.dispatch = function(options){	
	...
	options.attribute = this;
	options.attributeName = this.name;
	this.owner.dispatch(Event.AttributeChange, options);
};
...
WAF.DataSourceEmRelatedAttribute = Class.create('DataSourceEmRelatedAttribute');
...
WAF.DataSourceEmDeepSimpleAttribute = Class.create('DataSourceEmDeepSimpleAttribute');
...
WAF.DataSourceVarAttribute = Class.create('DataSourceVarAttribute');
...
WAF.DataSource = Class.create('dataSource');
...
WAF.DataSource.buildEventFromString = function(eventKind) {
	if (eventKind == 'all') {
		return Event.DataSource;
	}
	
	if (eventKind.substr(0, 2) == 'on') {
		eventKind = eventKind.substr(2);
	}
...
WAF.DataSource.getEventKind = function(eventKind) {
	if (eventKind.kind) {
		eventKind = eventKind.kind;
	} else {
		if (eventKind.constructor)
		eventKind = eventKind.constructor.kind;
	}
	eventKind = 'on' + eventKind;
	return eventKind;
}

WAF.DataSource.addListener = function(eventKind, eventHandler, config, userData){
	var eventID, evHandler, c, uD,  attributeName;
	config = config || {};
	userData = userData || {};
	...
	if (typeof eventKind == 'string') {
		eventKind = WAF.DataSource.buildEventFromString(eventKind);
	}
	
	if (!this._private.listenerStorage) {
		this._private.listenerStorage = [];
	}
	...
	if (attributeName) {
		this.subscribe(eventKind, attributeName, callback);
	} else {
		this.subscribe(eventKind, callback);
	}
...
WAF.DataSource.removeListener = function(params) {
	var id = -1,
	widgetId = -1,
	listeners = this._private.listeners,
	listeners = this._private.listenerStorage,
	found =-1,
	i = 0,
	...
	// search for id
	if (widgetId != -1 && found == -1) {
		for (i = 0; i < length; i++) {
			if (listeners[i].id == widgetId || 
			(listeners[i].userData && listeners[i].userData.widget && listeners[i].userData.widget.id == widgetId) ||
			(listeners[i].userData && listeners[i].userData.datagrid && listeners[i].userData.datagrid.id == widgetId)) {
				found = i;
				break;
			}
		}
	...
	if (found !== -1) {
		this.unsubscribe(listeners[found].realEvent, listeners[found].realCallback);
...
WAF.DataSource.removeAllListeners = function(){
	var i, l, listeners = this._private.listenerStorage;
	for(i = 0, l = listeners.length; i < l; i++) {
		this.unsubscribe(listeners[i].realEvent, listeners[i].realCallback);
	}
	this._private.listenerStorage = [];
};
...
WAF.DataSource.dispatch = function(eventKind, options){
	...
	if (eventKind == "onAttributeChange" || eventKind == Event.AttributeChange || Class.inheritFrom(eventKind, Event.AttributeChange)) {
		checkAttName = true;
	}
	if (!options.stopDispatch) {
		...
		if (typeof eventKind != 'string') {
			eventKind = WAF.DataSource.getEventKind(eventKind);
		}
		...
		if (eventKind) {
			if (checkAttName) {
				newEvent = new eventKind(attName, event);
			} else {
				newEvent = new eventKind(event);
			}
			this.fire(newEvent);
		}
	}
...
WAF.DataSourceEm = Class.create('DataSourceEm');
		...
		case "dataClass":
			...
			if (priv.initialQuery) {
				var qStr = priv.initialQueryStr ? priv.initialQueryStr : '', orderByStr = '', mainOptions = {
					pageSize: priv.dataClass.getDefaultTopSize() || 40,
...
WAF.DataSourceVar = Class.create('DataSourceVar');
