                ...
                if (WAF.PLATFORM.isTouch) {
                    switch (widget.att.type) {
                        case "date":
                            ...
                            else if (config["data-datapicker-on"] === "true") {
                                htmlObject.scroller();
                            }
                            break;
                        default:
            ...
            if (enumValueList.length > 0) {
                ...
                if (isParentMatrix.length == 0 || (typeof e != 'undefined' && e.subID && isParentMatrix.length != 0)) {
                    htmlObject.autocomplete({
                        source: function(request, response){
                            var url = '/rest/' + widget.source.getDataClass().getName() + '/' + widget.att.name + '?$distinct=true&$top=20';
                            
                            url += '&$filter=' + '"' + widget.att.name + '=\'' + request.term + WAF.wildchar + '\'"';
