...
module.exports = (function(){
    ...
    PackageListHelper = {
        ...
        diffPackageListStorageVsWpm: function(targetPackageList, sourcePackageList){
            ...
            console.log('>packageListHelper.diffPackageListStorageVsWpm() - start');
        
            //if nothing to merge
            if(Object.keys(sourcePackageList).length === 0){
                console.log('>>packageListHelper.diffPackageListStorageVsWpm() - nothing to merge');
                result.packageList = targetPackageList;
                return result;
            }
            //otherwise make a copy of the source
            else{
                console.log('>>packageListHelper.diffPackageListStorageVsWpm() - creating result from WPM.getPackageList()');
                result.packageList = JSON.parse(JSON.stringify(sourcePackageList));
            }
            
            console.info('>> >packageListHelper.diffPackageListStorageVsWpm() - start adding packages from storage not found in WPM.getPackageList() to result');
            ...
            for(targetPackageName in targetPackageList){
                if(!result.packageList[targetPackageName]){
                    console.info('>> >packageListHelper.diffPackageListStorageVsWpm() - adding package : ',targetPackageName);
                    result.packageList[targetPackageName] = targetPackageList[targetPackageName];
                }
            }
            console.info('>> <packageListHelper.diffPackageListStorageVsWpm() - stop adding packages from storage not found in WPM.getPackageList() to result');
            
            //2nd copy files from target over result which have different TS
            if(Object.keys(targetPackageList).length > 0){
                //loop through target
                console.info('>> >packageListHelper.diffPackageListStorageVsWpm() - start updating timestamps of result with the ones from WPM.getPackageList() + updating buildListChanges');
                for(targetPackageName in targetPackageList){
                    if(!result.packageList[targetPackageName]){
                        ...
                        for(targetFileIndex = 0; targetFileIndex < targetPackageList[targetPackageName].files.length; targetFileIndex++){
                            for(sourceFileIndex = 0; sourceFileIndex < sourcePackageList[targetPackageName].files.length; sourceFileIndex++){
                                ...
                                if(targetPackageList[targetPackageName].files[targetFileIndex].file === sourcePackageList[targetPackageName].files[sourceFileIndex].file){
                                    if(result.packageList[targetPackageName].files[targetFileIndex] && (new Date(result.packageList[targetPackageName].files[targetFileIndex].TS)).getTime() !== (new Date(targetPackageList[targetPackageName].files[targetFileIndex].TS)).getTime()){
                                        console.log('>>> >packageListHelper.diffPackageListStorageVsWpm() > file '+targetPackageList[targetPackageName].files[targetFileIndex].file+' has changed, updating TS in result + tagging as changed in buildListChanges');
                                        result.packageList[targetPackageName].files[targetFileIndex].TS = targetPackageList[targetPackageName].files[targetFileIndex].TS;
                ...
                console.info('>> <packageListHelper.diffPackageListStorageVsWpm() - stop updating timestamps of result with the ones from WPM.getPackageList() + updating buildListChanges');
            }
            console.log('<packageListHelper.diffPackageListStorageVsWpm() - stop');
