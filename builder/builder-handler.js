    ...
    try{
        ...
        if(storage.tryLock()){
            if(moduleBuildCores.minCoresFilesExist() === false){
                console.log('==== CORE.MIN > no core.min files > creating them ====');
                moduleBuildCores.makeMinCoresFiles(doNotMinify);
            }
            else{
                console.log('==== CORE.MIN > core.min files already here ====');
            }
            storage.unlock();
        }
        ...
        if(!urlInfos.debug && internal.getDebuggerServer() !== 1024){
            urlInfos.debug = true;
        }

        //try the hardCache way
        if(!urlInfos.debug && builderServiceSettings && builderServiceSettings["hardCache"] === "true"){
            //no storage lock !
            ...
            if(buildFromStorage && buildFromStorage[tmpFilePath]){
                ...
                console.log('hardcache - found : '+ tmpFilePath);
            }
            else{
                hardCacheMode = false;
            }
        ...
        response.body = "/** Response generated at "+((new Date()).toUTCString())+(hardCacheMode ? " (response directly from hardCache) " : "")+"*/\n\n"+output;
