        ...
        getInfos: function(request){
            ...
            if(referer){
                ...
                if(request.urlQuery.match(/useReferer=1/)){
                    ...
                    //escape special characters in ipV6 that will conflict with regexp
                    hostRegex = request.headers.HOST.replace(/\[|\]|\:/g,function(match){
                        return "\\"+match;
                    });
                    hostRegex = new RegExp('^'+hostRegex);
                    referer = referer.replace((hostRegex),function(match){return '';});
